...
| `ENGAGEMENT_HMAC_KEY` | HMAC key for pseudonymizing pivot values, asset addresses, and `account_id` written to the Triage engagement store (`engagement_impression` / `engagement_action`). Set to base64 of ≥32 random bytes (e.g. `openssl rand -base64 48`); the helper rejects shorter values at runtime. The key does **not** rotate — rotating would invalidate every historical row's join key, so future rotation requires an expand/contract migration that adds a `_key_version` column. See `src/lib/triage/engagement/hmac.ts`. |
