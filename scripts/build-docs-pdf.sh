...
trap 'rm -f mkdocs.tmp.yml; if [[ "${AICE_PDF_DEBUG:-0}" != "1" ]]; then rm -rf .pdf-tmp; fi' EXIT

AICE_LOCALE="$locale" "$python_bin" - <<'PY'
import copy
import os
import sys
...
locale = os.environ.get("AICE_LOCALE")
if not locale:
    print("AICE_LOCALE is required", file=sys.stderr)
    sys.exit(1)
...
pdf_plugin = {
    "with-pdf": {
        "enabled_if_env": "AICE_PDF_EXPORT",
        "output_path": os.path.join(root, "site", "pdf", f"aice-web-manual.{locale}.pdf"),
...
if locale == "ko":
    pdf_plugin["with-pdf"]["cover_title"] = "AICE Web"
...
AICE_PDF_EXPORT=1 "$mkdocs_bin" build -f mkdocs.tmp.yml
