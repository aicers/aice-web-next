...
Phase 2 sends Triage Baseline events, Stories, and Policy runs from aice-web-next to aimer-web so the latter can produce LLM-driven analyses and reports. Communication is one-way (aice-web-next → aimer-web, browser-mediated to respect aice-web-next's air-gapped deployment), incremental for streaming data (Baseline, Story), and on-demand for batched user-curated work (Policy run). aimer-web stores ingested data in a new query-friendly schema separate from the Phase 1 `detection_events` per-event redacted store. `detection_events` is no longer a routing destination of the Detection-menu Send button (which now goes through aimer-web's `/api/analysis/analyze-bridge` endpoint per [aicers/aimer-web#274](https://github.com/aicers/aimer-web/issues/274) and this repo's [#629](https://github.com/aicers/aice-web-next/issues/629)); it serves as the per-event redacted record that aimer-web's analyze flow dedupes against, written incidentally by that flow as a synthetic per-event ingest when an event keyed by `(aice_id, event_key)` is not yet present (regardless of which surface triggered the analysis), plus any pre-existing rows from the historical bridge-handoff path. The Phase 1 store itself was refactored by [aimer-web RFC 0001](https://github.com/aicers/aimer-web/blob/main/rfcs/0001-ai-analysis-storage.md) from a multi-event encrypted batch row into per-event rows with redacted JSONB + a shared `event_redaction_map`; see §"Existing: `detection_events`" below for the new shape. Cross-customer Story analysis is explicitly deferred to a Phase 3 RFC; the identifier policy in this RFC keeps that path forward-compatible.
...
1. **Air-gap respect** — aice-web-next servers cannot initiate outbound network traffic; aimer-web cannot poll aice-web-next. All cross-boundary movement happens through user browsers.
2. **One-way contract** — aimer-web cannot ask aice-web-next for anything. Whatever data aimer-web needs must arrive via push.
3. **Customer isolation** — both systems use per-customer databases. Phase 2 stays within that boundary; cross-customer correlation is Phase 3.

Phase 1 already handles the orthogonal use case of "send this single Detection event for one-shot analysis" — under the post-[aimer-web#274](https://github.com/aicers/aimer-web/issues/274) flow this is mediated by aimer-web's `/api/analysis/analyze-bridge` endpoint (see [#629](https://github.com/aicers/aice-web-next/issues/629) for the consumer-side rewire, which supersedes the original bridge-handoff path tracked at [#441](https://github.com/aicers/aice-web-next/issues/441)); the Phase 1 per-event redacted `detection_events` store remains distinct from the Phase 2 tables, written incidentally by aimer-web's analyze pipeline as a first-touch synthetic ingest. Phase 2 does **not** subsume Phase 1; it adds incremental sync for the structured Triage and Story data on top.
...
No longer has a Send-button-driven dedicated write path. Under the post-[aimer-web#274](https://github.com/aicers/aimer-web/issues/274) / [#629](https://github.com/aicers/aice-web-next/issues/629) flow, `detection_events` is written by aimer-web's analyze pipeline on first-touch (`source='manual'`, `connection_id=NULL`) for any event not yet present — irrespective of baseline status, and irrespective of which surface (Detection-menu Send button, same-origin operator UI, etc.) triggered the analyze. Pre-existing rows from the historical bridge-handoff path remain valid per-event redacted records. Refactored by [aimer-web RFC 0001](https://github.com/aicers/aimer-web/blob/main/rfcs/0001-ai-analysis-storage.md) from a multi-event encrypted batch row into **per-event redacted rows**:
...
Server-side redaction (regex-based v1 policy + per-customer public IP ranges) runs at ingest. Under the post-[aimer-web#274](https://github.com/aicers/aimer-web/issues/274) analyze-bridge flow the per-event ingest into `detection_events` is synthetic (`source='manual'`, `connection_id=NULL`) and bypasses the staged workflow entirely — aimer-web's `processBridgeCallback` skips the `staged_event_payloads` UPDATE and `staged_event_customers` INSERT loop when a `pending_analysis_requests` row is present. The staged-event approval workflow itself persists only for any pre-existing legacy bridge-handoff rows; there is no live aice-web-next-side producer for that path after [#629](https://github.com/aicers/aice-web-next/issues/629) lands. See aimer-web RFC 0001 as the canonical spec for column types, indexes, and the encryption envelope.
...
Phase 1's `detection_events`, as refactored by [aimer-web RFC 0001](https://github.com/aicers/aimer-web/blob/main/rfcs/0001-ai-analysis-storage.md), is now per-event redacted JSONB — so the "query-opaque encrypted blob" half of the original argument no longer applies. Phase 2 still uses separate tables for the remaining reason: a different access pattern (frequent indexed reads, joins, incremental sync state, batched aggregate rows) than the per-event redacted record `detection_events` maintains for the analyze flow's dedupe target. The two also differ in transport-time consent gating — Phase 2 batch endpoints run under one-time opportunistic-push consent ([#437](https://github.com/aicers/aice-web-next/issues/437)), whereas the Detection-menu Send button now flows through aimer-web's analyze-bridge ([aimer-web#274](https://github.com/aicers/aimer-web/issues/274), this repo's [#629](https://github.com/aicers/aice-web-next/issues/629)) gated by OIDC sign-in + JWS verification rather than per-item staged approval — but that distinction is about gating mechanics, not about why the storage schemas are separate.
...
- **Analyze defaults (forward note)**: per [#629](https://github.com/aicers/aice-web-next/issues/629) Decision 3, the `model_name` / `model` defaults that flow into the `analyze_params_token` JWS claims used by the Detection-menu Send button's analyze-bridge call come from two new fields on the same [#437](https://github.com/aicers/aice-web-next/issues/437) Aimer-integration settings UI (`aimer_default_model_name`, `aimer_default_model`). These configure the analyze-bridge transport, not the Phase 2 batch endpoints, and are documented here only so readers can locate where the analyze parameters live operationally.
...
**Phase 2 ingestion path is separate from Phase 1 staged workflow.** Phase 1's bridge route (`/api/auth/bridge`) historically wrote to `staged_event_payloads` and waited for explicit per-customer approval before promoting to `detection_events`; under the post-[aimer-web#274](https://github.com/aicers/aimer-web/issues/274) analyze-bridge flow the Detection-menu Send button no longer transits the staged workflow at all — `processBridgeCallback` skips the staging UPDATE/INSERT loop when a `pending_analysis_requests` row is present, and the analyze pipeline writes `detection_events` directly as a synthetic per-event ingest. Phase 2 endpoints (`/api/phase2/*`) verify the envelope using the same shared module but then **INSERT directly into the Phase 2 tables** (§5) with no staging, no approval gate. This is intentional: Phase 2 is incremental sync of structured data the user already consented to via the [#437](https://github.com/aicers/aice-web-next/issues/437) one-time consent (§8 Trigger UX & consent model), whereas Phase 1's live ad-hoc per-event send surface is now consent-gated by aimer-web's analyze-bridge entry (OIDC sign-in + JWS verification) rather than per-item staged approval. The structural argument here — Phase 2 reuses the Phase 1 envelope module via a separate ingestion path — is unchanged by either gating model.
...
Under the post-[aimer-web#274](https://github.com/aicers/aimer-web/issues/274) flow, the Detection-menu "Send this event to aimer-web" button routes via a **single transport** regardless of baseline status (per this repo's [#629](https://github.com/aicers/aice-web-next/issues/629)): a top-level multipart `<form>` POST with `target="_blank"` to aimer-web's `/api/analysis/analyze-bridge` endpoint, which absorbs the OIDC round-trip server-side and `302`s to an analysis result page in the new tab. Both the Phase 1 multipart bridge handoff and the Phase 2 immediate-push channel disappear from this button's call graph. See [aicers/aimer-web#272](https://github.com/aicers/aimer-web/issues/272) §"End-to-end flow with Q1–Q3 in place" for the authoritative cross-origin diagram.

The button's purpose has shifted accordingly: from "push this event into aimer-web storage" to "trigger AI analysis on this event." The storage side-effect — a row in `detection_events` keyed by `(aice_id, event_key)` when none is yet present — is now a property of aimer-web's analyze pipeline (synthetic per-event ingest with `source='manual'`, `connection_id=NULL`), not a routing decision on aice-web-next side. The same side-effect applies to **any** aimer-web analyze invocation that lands on a not-yet-present event, regardless of which surface triggered it.

The old design's invariant — "a baseline-passing event is **never** stored in `detection_events`" — no longer holds. Under the new flow, baseline-passing events analyzed via the Send button do acquire a `detection_events` row as a side-effect of aimer-web's `runAnalyzeFlow`. The actual post-[aimer-web#274](https://github.com/aicers/aimer-web/issues/274) invariant is weaker: a baseline-passing event's **canonical** storage is `baseline_event` (via opportunistic Phase 2 push), but it may **also** carry a `detection_events` row if any user triggers analysis on it. The two rows are consistent — both key by `event_key` and join to the same `event_redaction_map` — and the §"Read-side lookup" precedence handles the both-rows-present case transparently. No user-facing semantic regresses: the analysis cache is decoupled from the event row, and event content is fetched from whichever per-kind table holds it.

Phase 2 opportunistic push (the periodic background drain) is unaffected by this rewire and continues per §"Trigger UX & consent model". The per-Story and per-Policy-run manual Send buttons described in §"Manual trigger surfaces" below are also unaffected — they push Story / Policy-run rows to the Phase 2 batch endpoints (`/api/phase2/story/batch`, `/api/phase2/policy-run`), which is orthogonal to the analyze-bridge change.
...
6. **Send button race during baseline re-classification.** When a `baseline_version` bump re-classifies a previously baseline-failing event as baseline-passing, any existing Phase 1 per-event redacted row in `detection_events` for that `event_key` stays as-is. Such a row may have originated from the historical bridge-handoff path (legacy ingest) or — under the post-[aimer-web#274](https://github.com/aicers/aimer-web/issues/274) flow — as a synthetic ingest written by aimer-web's `runAnalyzeFlow` when a user triggered analysis on the event via the Detection-menu Send button. The Phase 2 row arriving via opportunistic push becomes the canonical analysis target, while any Phase 1 row for the same `event_key` (regardless of origin) remains a valid per-event redacted record (and is itself searchable by `event_key` per RFC 0001 — see §"Read-side lookup"). Whether to also emit an explicit "drop Phase 1 row" notice for storage cleanup is an optimization that can be added later; the behaviour does not change with the RFC 0001 refactor or the analyze-bridge flow.
...
- aimer-web cross-origin transport gap umbrella (Path A): [aimer-web#272](https://github.com/aicers/aimer-web/issues/272)
- aimer-web RFC 0001 amendment (analyze-bridge contract): [aimer-web#273](https://github.com/aicers/aimer-web/issues/273) → merged via [aimer-web#275](https://github.com/aicers/aimer-web/pull/275)
- aimer-web server-side analyze-bridge endpoint (merged 2026-05-20): [aimer-web#274](https://github.com/aicers/aimer-web/issues/274) → merged via [aimer-web#276](https://github.com/aicers/aimer-web/pull/276)
- aice-web-next consumer rewire against the analyze-bridge contract: [aice-web-next#629](https://github.com/aicers/aice-web-next/issues/629)
- This doc-sync record: [aice-web-next#628](https://github.com/aicers/aice-web-next/issues/628)
- Successor doc-sync record (analyze-bridge follow-up): [aice-web-next#632](https://github.com/aicers/aice-web-next/issues/632)
