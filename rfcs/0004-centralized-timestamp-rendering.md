...
- **A string-producing hook alongside the component** (e.g. `useTimestampFormatter`). A subset of call sites consumes the formatted value as **data**, not JSX, and cannot render a component: the shared `TriageEventTable` row contract (`stories-view`, `asset-detail` — caller-side formatting is contractual there), the breadcrumb label (`event-breadcrumb-registrar`, the sole `formatDateTimeCompact` site), chart/progress labels (`resource-sparkline`, `node-detail-dashboard`), and template-literal labels (`webauthn-card`). Those sites migrate to the hook or explicitly stay on the raw formatter; the migration plan must mark which, per site. The hook applies the same pre-mount deferral, but a bare string cannot carry the component's placeholder guarantees (`visibility: hidden`, `aria-hidden`, `<time dateTime>`) — so it returns resolution state rather than a magic placeholder string. Concretely it returns formatter functions plus a resolved flag (e.g. `{ resolved, format, formatCompact }`, the formatters returning `null` pre-mount) rather than a single pre-formatted value, because several consumers format per-row inside list mappings where a per-value hook cannot be called. The caller owns the null handling and whatever wrapper/announcement semantics its context allows. The acceptance criteria below bind the **component**, not hook consumers.
...
- Replace the 32 inline `formatDateTime` / `formatDateTimeCompact` call sites (22 files): JSX render sites take `<Timestamp>`; string-as-data sites take the hook (or are explicitly marked as staying on the raw formatter). Executed as a **single batch** — the API and all call sites in one issue (#764) dispatched to autonomous agents — rather than staged per-area PRs: per-issue CI/testing overhead of agent dispatch outweighs the reviewability benefit of small PRs for a mechanical migration, and the issue carries a per-site disposition table so no contract judgment is left to the agent.
...
- Visual review of the single migration PR to catch spacing/wrapper regressions where inline calls had bespoke markup; correctness is otherwise gated by the unit and E2E suites.
...
Execution order: wait for the aimer-side reference implementation (aicers/aimer-web#555) to land and settle the placeholder design; then deliver the API and the full call-site migration as a single batch (#764).
