...
This page covers the Settings menu and its sub-tabs: Accounts,
Roles, Customers, Policies, and Account Status. These are
administrative functions — the core threat detection features are
documented separately as they become available.
...
| Policies | `system-settings:read` |
| Account Status | `dashboard:read` |
...
| Policies | `system-settings:read`, `system-settings:write` |
...
![Policies](../assets/system-settings-en.png)
...
All changes to policies are recorded in the audit log.

## Account Status

Navigate to **Settings → Account Status** to view real-time
operational status of accounts, sessions, and certificates.
Requires `dashboard:read` to view.

![Account Status](../assets/account-status-en.png)
