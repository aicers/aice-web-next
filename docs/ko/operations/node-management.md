...
> **스크린샷 부채.** 수정 다이얼로그 UI 자체는 동일한 Phase Node-9 단계의
> 다른 서브 이슈에서 구현되며, 본 브랜치에서는 아직 렌더링할 수 없습니다.
> 따라서 이 절은 저장(Save)을 좌우하는 BFF 동작만 문서화합니다. 다이얼로그
> 와 재조정 프롬프트의 PNG 캡처는 다이얼로그를 구현하는 동일한 후속 PR에서
> `docs/AUTHORING.md` 가 안내하는 모킹된 GraphQL 엔드포인트 캡처 절차에
> 따라 추가됩니다.
...
편집을 **편집 가능한 필드 단위로** 새 기준 위에 다시 정렬한 뒤 한 번만 자동
재시도합니다. 재정렬은 행 단위가 아니라 필드 단위입니다. 예를 들어 사용자가
어떤 서비스의 draft 문자열만 수정하고 다른 사용자가 같은 서비스의 status를
뒤집었다면, 재시도 페이로드는 사용자의 draft와 다른 사용자의 status를 함께
보냅니다. 프로필 하위 필드(고객/설명/호스트명) 도 동일한 규칙이 적용됩니다.
한 번의 재시도로 성공하면 사용자에게는 별도 안내가 표시되지 않고 저장이
정상적으로 완료됩니다.

재시도까지 또 다시 충돌하면(그 사이 또 다른 사용자가 저장한 경우) 다이얼로그
는 진행을 멈추고 재조정 프롬프트를 두 가지 옵션과 함께 표시합니다.
