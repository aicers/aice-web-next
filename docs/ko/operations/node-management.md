...
![센서 에이전트가 유휴 상태인 상태 행 (와이어프레임 대체)](../../assets/node-status-legend-ko.svg)

> 위 캡처는 실제 백엔드에서 유휴 상태의 상태 행을 촬영할 수 있을
> 때까지 사용하는 SVG 와이어프레임입니다(현재 작성 환경에서는 실제
> 배포 환경의 `RELOAD_FAILED` 에이전트를 재현할 수 없습니다). 추후
> 후속 작업에서 PNG 캡처로 교체될 예정입니다.
