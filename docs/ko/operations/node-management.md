...
매니저 알림, 그리고 각 외부 서비스가 실행됩니다. 어느 단계든 실패하면
fan-out 은 멈추며, 다음 디스패치는 직전 디스패치가 성공해야만 실행됩니다.
두 매니저 단계는 독립적으로 관측되고 재시도 가능합니다. 운영자가 DB 단계가
이미 성공한 행의 알림 디스패치만 재시도하면 DB mutation 은 다시 실행되지
않습니다.
