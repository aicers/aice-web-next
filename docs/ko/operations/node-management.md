...
> **스크린샷 부족.** 모달은 아직 노드 상세 페이지에 마운트되지
> 않았습니다 — 상세 페이지는 Phase Node-5 (#311) 가 소유하며,
> 이 PR의 e2e 하니스가 사용해야 할 모의 매니저 / 외부 엔드포인트는
> 별도 트랙입니다. 예정된 디스패치 목록, Retry 버튼이 노출된
> `failed_retryable` 행, Rebuild 안내가 노출된 `failed_terminal`
> 행의 실제 PNG 캡처는 `docs/AUTHORING.md` 의 인프라 의존 기능에
> 대한 스크린샷 예외에 따라 상세 페이지 마운트와 함께 수록됩니다.
