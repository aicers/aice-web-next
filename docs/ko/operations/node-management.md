...
![정리 엔드포인트 요청과 응답 예시](../../assets/node-apply-cleanup-response-ko.svg)
...
2. 모든 BFF 인스턴스와 라우트를 호출하는 스케줄러에 환경 변수를
   설정합니다. 세 TTL 노브의 기본값은 `.env.example` 에 포함되어
   있으며 런타임에 읽히므로, 환경 변수를 미설정 상태로 두는 것은
   "문서화된 기본값을 사용한다" 는 지원되는 경로입니다.
