...
- Escape 와 모달 외부 클릭이 무시됩니다 — 진행 중인 BFF 호출은
  ...
  상태로 고립됩니다.
- DB 단계가 실행되는 동안에는 **매니저 DB** 행만 **진행 중** 으로
  프로젝션합니다. DB 단계가 성공하면 모든 나머지 DB 이후 행 (매니저
  알림 및 각 외부 서비스) 을 동시에 **진행 중** 으로 프로젝션합니다.
  이는 BFF 의 DB 이후 병렬 fan-out (`apply-attempt-lifecycle.ts` 의
  `commitDbSuccessAndFanOut`) 을 그대로 반영합니다. 각 행은 자신의
  디스패치가 정착되는 대로 독립적으로 **성공** / **실패** 로
  넘어갑니다.
- 액션 버튼에 **적용 중…** 라벨이 표시됩니다.

각 행은 다섯 가지 상태 중 하나로 표시됩니다. **대기 중**은 사용자가
적용을 클릭하기 전 (예정 목록 뷰), 그리고 DB 단계가 아직 진행 중인
동안 DB 이후 디스패치에서 나타날 수 있습니다 — DB 단계 실패는
시퀀스를 정지시키고 DB 이후 디스패치는 DB 단계 재시도 성공 시 재개를
기다리며 `queued` 로 남기 때문입니다.

| 상태 | 의미 |
| :-- | :-- |
| **대기 중** | 아직 시작되지 않음 — 적용 클릭 전 예정 목록, 그리고 DB 단계가 아직 진행 중인 동안 DB 이후 디스패치에 모두 나타납니다. DB 단계가 성공하면 모든 나머지 DB 이후 디스패치가 병렬로 **진행 중** 으로 승격됩니다. |
| **진행 중** | 디스패치가 실행 중입니다. `confirmApplyAttempt` 호출이 진행 중일 때 모달은 먼저 DB 행에, 이어서 DB 단계가 성공한 직후 모든 DB 이후 행에 병렬로 이 상태를 프로젝션합니다. `retryDispatch` 호출이 진행 중일 때는 재시도된 행에만 프로젝션합니다. |
| **성공** | 디스패치가 정상 반환되었습니다. |
