...
대기 상태는 비교 기반으로 판정합니다. 에이전트는 `agent.draft` 와
`agent.config` 를, 외부 서비스는 매니저 측 `draft` 와 페이지 로드 시
가져온 엔드포인트의 실제 `config` 를 구조적으로 비교합니다. 적용 값과
구조적으로 동일하게 회귀한 초안은 안정 상태로 간주되어 대기 표시가
나타나지 않습니다.

비교 결과 변경이 검출된 행은 다음과 같이 표시됩니다.

- 왼쪽 가장자리에 **대기 중(Pending)** 배지.
- `이름`, `고객`, `설명`, `호스트명` 중 초안 값이 다른 셀은 두 줄로 표시되며
  적용 값은 취소선이 그어지고 그 아래에 초안 값이 나타납니다.
- 서비스 상태 아이콘 옆에 작은 황색 점이 표시되어 해당 서비스가 대기
  상태임을 알립니다.
- 외부 서비스(Data Store / Tivan) 의 페이지 로드 읽기가 실패한 경우, 해당
  열의 상태 아이콘 옆에 회색 **알 수 없음** 점이 표시됩니다. 연결할 수
  없는 엔드포인트에 대해서는 대기 상태를 단정할 수 없으므로 "변경
  없음" 으로 간주하지 않고 알 수 없음 상태를 명시적으로 표기합니다.
...
Central Manager 의 GraphQL 표면은 유일성 / 범위 충돌에 대해 평문 오류 메시지를
...
3. **외부 서비스 단계.** 적용 빌드 시점에 비-null `draft` 가 있던 각 외부
   서비스(Data Store 는 `DATA_STORE`, Tivan 은 `TI_CONTAINER`) 에 대해 상위
...
DB 단계가 가장 먼저 실행되며 이후 모든 단계를 게이트합니다. DB 단계가
실패하면 알림이나 외부 서비스 디스패치는 시도되지 않습니다. **DB
단계가 성공하면 알림 디스패치와 모든 외부 `updateConfig` 디스패치가 병렬로
실행됩니다.** DB 이후의 각 디스패치는 자체 per-dispatch claim 을 보유하고
각자의 상태를 독립적으로 커밋합니다. 알림 실패가 외부 서비스의 진행을
지연시키지 않으며, 한 외부 서비스의 실패가 다른 외부 서비스를 지연시키지
않고, 느린 디스패치가 더 빠른 디스패치의 완료를 막지도 않습니다. 행 전체
상태(`succeeded` / `failed_retryable` / `failed_terminal`)는 모든 DB
이후 디스패치가 정착된 후 디스패치별 결과의 집계로 커밋됩니다. 두 매니저
단계는 독립적으로 관측되고 재시도 가능합니다. 운영자가 DB 단계가 이미 성공한
행의 알림 디스패치만 재시도하면 DB mutation 은 다시 실행되지 않습니다.
...
2. **노드 단위 범위 검사(및 존재 확인).** 그 다음 래퍼는 **여전히 디
   ...
   *현재* 범위와 비교하고, 전역 범위 호출자
   ...
   디스패처는 배포 전체에 걸친 Data Store / Tivan 엔드포인트와 통신하며
   ...
   *삭제된* 노드에 대해 `updateConfig` 를 계속 발송하면서 그 노드에 대한
   ...
   할 수 없습니다. 테넌트 범위 호출자에 대해서는 Central Manager 이 범위 밖
...
외부 서비스마다 `updateConfig` 가 한 번씩 실행됩니다. Central Manager
...
이 BFF 호출은 일회성입니다 — 디스패치별 진행 상태를 클라이언트로
스트리밍하지 않습니다. 호출 진행 중 모달은 다음과 같이 동작합니다.

- Escape 와 모달 외부 클릭이 무시됩니다 — 진행 중인 BFF 호출은
  취소할 수 없기 때문에, 중간에 UI 를 닫으면 행이 `executing`
  상태로 고립됩니다.
- 첫 번째 대기 중 행 (**매니저 DB** 디스패치) 을 낙관적으로
  **진행 중** 으로 프로젝션해 적용이 실행 중임을 알립니다. 호출이
  진행 중인 동안 다른 행은 모달 뷰에서 **대기 중** 상태로 남습니다.
  BFF 의 DB-성공 핸드오프와 그 이후의 병렬 post-DB fan-out 은
  스트리밍되지 않기 때문에 모달이 그 전이를 그대로 반영할 수
  없습니다.
- 액션 버튼에 **적용 중…** 라벨이 표시됩니다.

`confirmApplyAttempt` 가 응답을 반환하면, 모달은 BFF 가 돌려준
디스패치별 최종 상태를 그대로 렌더링합니다. 익스큐터가 post-DB
fan-out 을 끝까지 실행한 뒤 반환하므로, 응답 시점에 모든 post-DB
디스패치는 이미 `succeeded`, `failed_retryable`, 또는
`failed_terminal` 중 하나로 정착해 있습니다. 운영자는 행별 최종
결과를 보게 되며, "DB 성공, post-DB 실행 중" 같은 중간 상태는
모달에 노출되지 않습니다.

각 행은 다섯 가지 상태 중 하나로 표시됩니다. **대기 중**은 사용자가
적용을 클릭하기 전 (예정 목록 뷰), 그리고 `confirmApplyAttempt`
호출이 진행 중일 때 모달의 낙관적 프로젝션에서 DB 가 아닌 행들에
표시됩니다.

| 상태 | 의미 |
| :-- | :-- |
| **대기 중** | 아직 시작되지 않음 — 적용 클릭 전 예정 목록, 그리고 `confirmApplyAttempt` 가 진행 중인 동안 DB 가 아닌 행에 모두 나타납니다. |
| **진행 중** | 디스패치가 실행 중입니다. `confirmApplyAttempt` 호출이 진행 중일 때 모달은 이 상태를 **매니저 DB** 행에만 프로젝션합니다 (BFF 의 DB-성공 / 병렬 post-DB 전이는 클라이언트로 스트리밍되지 않습니다). `retryDispatch` 호출이 진행 중일 때는 재시도된 행에만 프로젝션합니다. |
| **성공** | 디스패치가 정상 반환되었습니다. |
...
- **다시 시도 (Retry)** — 디스패치가 `failed_retryable` 상태일
  ...
  `retryDispatch({ attemptId, dispatchId })` 를 호출합니다. DB
  이후 fan-out 은 알림과 외부 서비스 디스패치를 각자의 claim 하에서
  병렬로 실행하므로 한 시도가 같은 행 위에서 두 개 이상의
  `failed_retryable` 디스패치로 정착할 수 있으며, 모달은 각 실패
  행마다 독립적인 다시 시도 버튼을 렌더링합니다. 재시도는 **오직**
  대상 디스패치만 다시 실행하며, 다른 형제 디스패치의 상태 (다른
  `failed_retryable` 행 포함) 는 관측된 그대로 보존됩니다.
...
상세 페이지가 열려 있는 동안 대시보드 이동 경로는
**노드 › {노드 이름}**(예: `노드 › Edge-01`)을 표시하며, 불투명한
노드 ID 대신 페이지 제목과 동일한 표시 이름을 사용합니다. 노드
데이터가 로드되기 전에는 정적 라벨 **노드 상세**로 대체됩니다.
...
버퍼가 데이터의 출처가 됩니다. 비교 기반(이름 / 프로필 / 에이전트 /
외부 서비스) 으로 한 항목이라도 적용 값과 다르면 **변경 대기 중**
배지가 노출됩니다. 대기 신호가 페이지 로드 읽기에 실패한 외부
서비스뿐인 경우에는 대신 **변경 대기 상태 알 수 없음** 배지가
표시되며, **전체 적용** 버튼은 외부 서비스가 응답할 때까지 툴팁과
함께 비활성화됩니다. 대시보드의 컨트롤(`수정`, `재시작`, `종료`,
...
- 상태 탭과 동일한 공유 폴링 버퍼로 구동되는 상태 배지
  (`On` / `Off` / `Idle`).
- 비교 규칙으로 변경이 검출될 때 표시되는 **변경 대기 중**
  배지(주황색). 에이전트는 `draft` 와 `config` 를, 외부 서비스는
  매니저 측 `draft` 와 페이지 로드 스냅샷의 엔드포인트 `config` 를
  비교합니다.
- 페이지 로드 시 해당 외부 서비스에 대한 엔드포인트 읽기가 실패한
  경우 외부 카드에 표시되는 **변경 대기 상태 알 수 없음**
  배지(회색). 이때 적용됨 탭을 표시할 수 없고 카드별 대기 상태도
  단정할 수 없습니다.
...
차이 탭은 비교 결과에 따라 셀 단위로 렌더링됩니다.

- **안정 상태** (에이전트는 `draft == config`, 외부는
  `manager.draft == endpoint.config`) 에서는 문서화된 문구
  `"이 서비스에는 적용 대기 중인 변경이 없습니다."` 를 표시합니다.
- **변경 의도** (`draft` 가 비어 있지 않고 구조적으로 다른 경우) 에는
  적용됨 / 드래프트의 필드 단위 비교 표가 표시됩니다.
- **삭제 의도** (적용 값은 있으나 `draft = null`) 에는 비교 표 위에
  삭제 마커 `"다음 적용 시 이 서비스가 제거됩니다."` 가 표시되어,
  적용 시 어떤 필드가 제거될지 운영자가 미리 확인할 수 있습니다.
...
- **외부 서비스**(데이터 저장소, TI 컨테이너)는 폴링 주기마다 해당
  ...
  **켜짐**, 어떤 형태든 오류(연결 거부, HTTP 500, GraphQL `errors[]`,
  ...
  유휴 상태가 없습니다. Data Store 와 Tivan 이 같은 틱에 호출되지 않도록
