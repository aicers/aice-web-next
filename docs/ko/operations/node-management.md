...
- Escape 와 모달 외부 클릭이 무시됩니다 — 진행 중인 BFF 호출은
  ...
  상태로 고립됩니다.
- 아직 `대기 중` 인 모든 행을 **진행 중** 으로 승격하여, BFF 가
  현재 계획을 처리 중임을 사용자가 확인할 수 있도록 합니다. 호출이
  반환되면 정착된 각 디스패치 상태가 이 임시 표시를 대체합니다.
- 액션 버튼에 **적용 중…** 라벨이 표시됩니다.

각 행은 다섯 가지 상태 중 하나로 표시됩니다. **대기 중**은 사용자가
적용을 클릭하기 전 (예정 목록 뷰) 뿐만 아니라, 정착된
`failed_retryable` 시도 이후에도 나타날 수 있습니다 — #359 의 순차
진행 규칙에 따라 실패가 시퀀스를 정지시키고 후속 디스패치는
재시도 성공 시 재개를 기다리며 `queued` 로 남기 때문입니다.

| 상태 | 의미 |
| :-- | :-- |
| **대기 중** | 아직 시작되지 않음 — 적용 클릭 전 예정 목록과, `failed_retryable` 행 이후 재개 규칙이 진행시키지 않은 디스패치에 모두 나타납니다. |
| **진행 중** | 디스패치가 실행 중입니다 (또는 `confirmApplyAttempt` / `retryDispatch` 호출이 진행 중일 때, 모달이 "BFF가 현재 처리 중" 임을 표시하는 프로젝션입니다). |
| **성공** | 디스패치가 정상 반환되었습니다. |
