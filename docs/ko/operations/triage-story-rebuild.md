...
1. **동일 윈도우에 베이스라인 재빌드를 실행한 직후.**
   베이스라인 재빌드(`POST /api/internal/triage/baseline/rebuild`)는
...
`POST /api/internal/triage/baseline/rebuild`로 한 윈도우의
...
1. 원하는 윈도우로 `POST /api/internal/triage/baseline/rebuild`를
   호출합니다. 응답(`status: 'ok'`) 또는 형식화된 오류를
   기다립니다.
2. 동일한 `customer_id` / `from` / `to`로
   `POST /api/internal/triage/story/rebuild`를 호출합니다. 200 응답을
   기다립니다.
