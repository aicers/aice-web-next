...
선별 메뉴의 Story 탭은 고객별 `event_group` /
...
`(correlation_rule_id, primary_asset, correlation_key, time_window_start, time_window_end)`로
...
`correlation_key` 구성요소는 다중 출발지 규칙에서 매칭을 모호하지
않게 만듭니다 — R5 캠페인 Story에는 `primary_asset`이 없고, 같은
피해 자산·윈도우의 두 R4 팬인 Story는 공격 카테고리로만 구분될 수
있습니다. 자산 기반 규칙 R1/R3에서는 `NULL`이며, 이들의 이월 동작은
변경되지 않습니다.

이 동작은 가장 흔한 재빌드 트리거인 "룰 변경으로 재계산"에
대응합니다 — 같은 윈도우/자산/룰 내에서 새 Story는 동일한 분석
단위를 표현하며, 운영자의 "이미 분석함" 인식이 유지되어야 합니다.
Clumit Insight 인테이크 계약(#492)은 콘텐츠가 실질적으로 바뀌어
재분석을 원할 때를 위해 `force_refresh: true` 명시적 이스케이프 해치를
이미 제공합니다.

자연 키는 `event_group`의 부분 UNIQUE 인덱스와 일치합니다 — R1/R3는
자산 인덱스(`(rule, asset, start, end) WHERE kind = 'auto_correlated'
AND primary_asset IS NOT NULL AND correlation_key IS NULL`), R4/R5는
`correlation_key` 인덱스(`(rule, correlation_key, start, end) WHERE …
correlation_key IS NOT NULL`)입니다. 따라서 큐레이션 Story는 구조적으로
이월 대상에서 제외됩니다.
...
- **Clumit Insight 푸시 부수 효과.** Story 재빌드는 이전에 전송된
  Story가 교체되었음을 Clumit Insight에 통지하지 않습니다. 해당
  정책은 포괄적인 선별/Story Clumit Insight 푸시 설계(우산 이슈
  #491)의 일부로 선별/Story 전반에 걸쳐 일관되게 다룹니다.
