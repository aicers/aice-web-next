...
이벤트 페이지는 사이드바에서 접근합니다. Data Store가 수집한 **원본
이벤트** — 백엔드가 수집한 원시 레코드로, 탐지 로직이 실행되기 전의
데이터 — 를 조회합니다. Data Store의 레코드 유형 34종을 모두 다룹니다:
...
!!! note "와이어프레임 대체본"
    ...
    Data Store에서 받은 데이터를 표시하므로, 실제 데이터가 적재된 스택에서
...
**적용**을 누르기 전까지는 아무것도 조회하지 않습니다 — Data Store는 모든
네트워크 질의를 정확히 하나의 센서로 한정하므로 센서 선택이
필수입니다.

- **레코드 유형** — 조회할 원본 이벤트의 종류입니다. Data Store의 레코드
  ...
  표시합니다(아래 참조).
- **센서** — 질의할 단일 센서입니다. 목록은 Data Store가 데이터를 수집한
...
- **에이전트 ID** — **Sysmon 유형에서만** IP·포트 범위 대신
  ...
  한정되므로, 에이전트 id에 대한 자유 텍스트 일치입니다(Data Store는
...
별도의 프로토콜 필터는 없습니다. Data Store의 네트워크 필터에는 프로토콜
...
바이트·패킷 수와 지속 시간은 Data Store가 문자열로 반환하는 64비트 값이며,
...
**레코드 유형** 필터에서 Data Store의 레코드 유형 34종을 모두 선택할 수
...
!!! note "와이어프레임 대체본"
    ...
    위 그림은 실제 캡처가 아닌 SVG 와이어프레임입니다. 차트는 Data Store에서
...
- **프로토콜** — 통계 API가 추적하는 프로토콜의 선택적 하위 집합입니다
  ...
  모두 선택하지 않으면 전체가 포함됩니다. Data Store가 다른 프로토콜 값을
...
X축은 버킷 시간입니다. Data Store는 각 버킷의 타임스탬프를 에폭 나노초
...
- **샘플링 정책** — 그릴 계열을 고르는 단일 선택 드롭다운입니다. 옵션은
  Central Manager에 정의된 샘플링 정책이며, 각 옵션의 라벨은 정책 이름입니다.
  Data Store는 시계열을 정책 id로 식별하므로 **적용**이 활성화되려면 정책을
...
이벤트 메뉴의 나머지와 동일한 게이트입니다. 정책 목록은 Central Manager에서,
계열 자체는 Data Store에서 가져오며 추가 권한은 필요하지 않습니다.
