...
실행되기 전의 데이터 — 를 조회합니다. Giganto의 네트워크 레코드 유형
20종 — 연결(**Conn**)과 [레코드 유형](#레코드-유형)에 나열된 19개
프로토콜 유형 — 을 모두 다루며, 각 유형마다 알맞은 열과 전체 행 상세를
제공합니다.
...
- **레코드 유형** — 조회할 원본 이벤트의 종류입니다. Giganto의 네트워크
  레코드 유형 20종을 모두 선택할 수 있습니다([레코드
  유형](#레코드-유형) 참조). 변경한 유형은 **적용**을 누를 때 반영되어
  검색이 다시 실행되고 결과 열과 상세 레이아웃이 해당 유형에 맞게
  교체됩니다.
...
**레코드 유형** 필터에서 Giganto의 네트워크 레코드 유형 20종을 모두
선택할 수 있습니다.
