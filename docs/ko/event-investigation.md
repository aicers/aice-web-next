...
![세계 지도가 포함된 엔드포인트 탭 — 와이어프레임](../assets/event-investigation-endpoints-map-ko.svg)

위 그림은 본 페이지의 다른 와이어프레임과 동일한
인프라 미비 상황의 스크린샷 예외 규정을 따라 제공되는 SVG
와이어프레임 대체물입니다. 스테이징 환경이 준비되는
대로 PNG 캡처(`event-investigation-endpoints-map-ko.png`)로
교체하세요.
