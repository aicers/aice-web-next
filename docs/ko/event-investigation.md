...
이 페이지는 이전에 `/events/<eventToken>` 경로에서 제공되었습니다.
토큰이 안정적이므로 북마크, 공유 메시지, 열린 탭에 그 옛 경로의
링크가 남아 있을 수 있습니다. 이러한 링크는 쿼리 문자열
(`returnTo`, `customers`, `aimerForce`)을 보존한 채 현재
`/detection/events/<eventToken>` 경로로 리디렉션되며, 리디렉션이
대시보드 콘텐츠가 렌더링되기 전에 처리되므로 **탐지** 메뉴가
강조 상태를 유지합니다.
