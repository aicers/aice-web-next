...
이름이 접근 가능한 센서에 더 이상 매핑되지 않는 경우(자산의 고객
범위 아래에서 일치가 0건이거나, REview가 세션 중 범위를 좁혀
해석된 `nodeId`를 거부한 경우) 경로는 자산 루트로 되돌아가고
페이지는 공유된 URL이 만료된 경우와 동일한 비차단 안내를
표시합니다. 전송/일반 오류는 여전히 빨간색 오류 안내로 표시되어
운영자가 "더 이상 접근 불가"와 "조회가 실행되지 않음"을 구분할
수 있습니다.
