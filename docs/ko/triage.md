...
`MIN_NONZERO_FLOOR` 폴백)을 적용한 뒤, 남은 행을 `orig_addr`로
그룹화하여 자산별 점수, 트리아지 카운트, `last_event_time`을
산출합니다. 메뉴 구성에서 살아남지 못한 행으로 자산이 상위에
오르는 일은 없습니다 — 분석가가 보는 목록을 할당량, 컷오프,
`MIN_NONZERO_FLOOR` 폴백이 끝까지 결정합니다. 다중 고객 범위는
고객마다 한 번씩 메뉴 코호트 SELECT를 실행하고, 테넌트별 자산
슬라이스를 이슈 계약의 `score DESC, last_event_time DESC` 키로
자바스크립트에서 머지하므로 점수가 같은 다른 테넌트의 행은
도착 순서가 아니라 최신성으로 정렬됩니다. 다중 고객 코드
경로는 `OFFSET`을 발행하지 않으므로 테넌트 간 전역 정렬이
안정적으로 유지됩니다.
