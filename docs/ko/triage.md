...
2. **테넌트 간 `final_menu_rows` 한도.** 테넌트별 §4 / §6 구성이
   ...
   행이 아니라 우선순위가 가장 낮은 행이 먼저 제거됩니다. 그래서
   화면에 보이는 자산 목록과 피벗 코퍼스가 동일한 우선순위 랭킹
   위에서 정렬을 유지합니다. 실제로는 §6 곡선이 코호트 크기에
   대해 로그 함수로 증가하므로 단일 테넌트의 슬라이스가 이 천장에
   닿는 일은 거의 없습니다 — 본 상한은 일상적인 한도라기보다 다중
   방어선 안전 장치에 가깝습니다.
