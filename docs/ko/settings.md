...
- **전역 제외 항목** — **설정 → 트리아지 제외 항목 → 전역**에서
  관리합니다. 모든 활성 고객에게 적용되며,
  `triage:exclusion:global:write` 권한이 필요합니다.
- **고객별 제외 항목** — `customer_id` 쿼리 파라미터와 함께
  **설정 → 트리아지 제외 항목**에서 관리합니다. 한 고객의 테넌트
  데이터베이스에만 적용됩니다. 조회는 `triage:read` 권한,
  편집은 `triage:exclusion:write` 권한과 해당 고객이 호출자의
  유효 범위에 포함될 것을 요구합니다.
...
> **스크린샷은 추후 추가 예정.** 이 섹션은 현재 텍스트만
> 제공합니다. **설정 → 트리아지 제외 항목** 트리의 라이브 캡처
> 환경이 마련되는 대로 후속 스크린샷 작업에서 목록 화면과 추가
> 다이얼로그의 UI 캡처가 추가될 예정입니다.
...
세 가지 미리보기 중 하나를 표시합니다:

- **접미사 `.example.com`로 축약** —
  `^.*\.example\.com$`이나 `^([a-z0-9-]+\.)*example\.com$` 같은
  패턴은 호스트명 접미사로 축약됩니다. 이 제외 항목은 `host` 또는
  `dns_query`가 해당 접미사로 끝나는 과거 코퍼스 행을 삭제합니다.
- **정확한 호스트명 `foo.example.com`로 축약** —
  `^foo\.example\.com$` 패턴은 정확한 호스트명으로 축약됩니다.
  해당 호스트명을 가진 과거 코퍼스 행이 제거됩니다.
- **전체 정규식 전용** — 그 외 모든 경우(분기, 앵커된 접두사,
  중간 와일드카드 등). 이 제외 항목은 **이후 케이던스 틱**에는
  적용되지만 과거 코퍼스 행을 소급해서 삭제하지는 않습니다.
  운영자가 놀라지 않도록 다이얼로그가 이를 명시합니다.
...
| **소급 적용 (ADD)** | 고객별 제외 항목 추가는 `baseline_triaged_event`와 `observed_event_meta`(코퍼스 B 테이블이 존재하면 `policy_triaged_event`까지)에 대해 명령당 10,000행씩 배치로 `DELETE`를 실행합니다. **전역** 제외 항목 추가는 고객별 팬아웃 작업을 큐잉하며, 워커 라우트가 각 고객의 케이던스 자문 잠금 아래에서 작업을 처리합니다. |
