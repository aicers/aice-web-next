...
이 페이지에서는 설정 메뉴와 하위 탭인 계정, 역할, 고객,
정책, 계정 현황을 다룹니다. 이들은 관리 기능이며, 핵심 위협
탐지 기능은 구현이 완료되는 대로 별도로 문서화됩니다.
...
| 역할 | `roles:read` |
| 고객 | `customers:read` |
| 정책 | `system-settings:read` |
| 계정 현황 | `dashboard:read` |
...
| 정책 | `system-settings:read`, `system-settings:write` |
...
모든 정책 변경 사항은 감사 로그에 기록됩니다.

## 계정 현황

**설정 → 계정 현황**으로 이동하여 계정, 세션, 인증서의
실시간 운영 상태를 확인합니다. 조회하려면 `dashboard:read`
권한이 필요합니다.

![계정 현황](../assets/account-status-ko.png)
