...
- **이름** — 고객 표시 이름(필수).
- **설명** — 선택적 설명.
- **External Key** — Clumit Insight의 일치하는 고객과
  페어링하는 교차 시스템 브리지 식별자(선택). 전 시스템에서
  globally unique 합니다. 해당 고객이 *Insight로 분석*에 아직
  온보딩되지 않은 경우 비워둡니다. 합의 및 검증 규칙은 아래
...
External key는 AICE 고객과 Clumit Insight 측의 일치하는 고객을
페어링하는 운영자 지정 식별자입니다. 양쪽 시스템이 동일한 값을
보유하므로 교차 시스템 브리지가 감사 / 이벤트 트래픽을 단일
비즈니스 엔티티로 매핑할 수 있습니다.

- **언제 설정하나요?** Clumit Insight System Administrator와
  out-of-band secure channel(사내 SSO 메신저, 직접 대면 등)로 값을
  사전 합의한 후에만 설정합니다. 권장 식별자는
  도메인(예: `acmecorp.com`), 사업자등록번호 또는 계약 코드입니다.
...
- **변경의 영향.** External key를 설정하거나 변경하면 교차 시스템
  매핑이 재설정됩니다. 매핑을 유지하려면 Clumit Insight 측의
  일치하는 고객 정보도 함께 업데이트해야 하며, 직후에 단일 bridge
  테스트로 매핑을 검증하는 것을 권장합니다.
- **해제의 영향.** External key를 해제하면 다시 설정할 때까지
  해당 고객의 *Insight로 분석*가 비활성화됩니다. Clumit Insight
  측과의 기존 매핑은 이 측에서는 더 이상 도달할 수 없습니다.
- **External key가 없는 고객.** 일반적인 편집 및 조회는 그대로
  동작하며, 고객별로 *Insight로 분석* 버튼만 값이 채워질 때까지
  비활성화됩니다.

전체 운영자 플레이북(합의 절차, 불일치 복구, 감사 포렌식)은
Clumit Insight의 표준 가이드인
...
언어 및 시간대 선택기 아래의 **시간 표시 형식** 섹션에서 앱 전반의 날짜와
시간 표시 방식을 선택할 수 있습니다. 네 가지 옵션은 서로 독립적이며 조합에
따라 다양한 구체적 형식을 만들어 냅니다. 옵션을 변경하면 샘플 시각의 **실시간
미리 보기**가 함께 갱신됩니다. 옵션은 브라우저의 `Intl` 포매터를 사용하므로 월
*이름*은 표시되지 않습니다(숫자 월만 표시).

- **표시 로캘** — *브라우저 따름*(기본값), *앱 언어 따름*, 또는 선별된 목록에서
  명시적으로 선택한 로캘. 날짜 순서, 구분자, 오전/오후 표기를 결정합니다.
- **시간 체계** — *로캘 따름*(기본값), *12시간제*, 또는 *24시간제*.
- **초** — *표시*(기본값) 또는 *숨김*.
- **시간대 표시** — *표시*(예: `GMT+9`) 또는 *숨김*(기본값).

옵션을 변경하지 않으면 타임스탬프는 이전과 동일하게 표시됩니다. 기본값은 기존
고정 형식과 바이트 단위로 동일합니다.

저장하면 페이지를 새로 고치지 않고도 화면에 이미 표시된 모든 타임스탬프에 새
형식이 즉시 적용됩니다.

> **참고:** 축약형 타임스탬프(예: 경로 표시줄)는 이 설정과 관계없이 항상 초와
> 시간대 표시를 생략하며, 표시 로캘과 시간 체계만 따릅니다.

![시간 표시 형식 환경설정](../assets/preferences-time-format-ko.png)
...
**설정 → Clumit Insight 연동**으로 이동하여 Insight로 분석 흐름에
필요한 시스템 전역 사전 조건을 설정합니다. 이 화면은 **System
...
Insight로 분석은 다섯 가지 시스템 전역 사전 조건을 요구합니다:

1. **`aice_id`** — 배포 호스트네임. JWT `iss` 클레임과 Clumit
   Insight로 전달되는 `aice_id` 클레임에 사용됩니다. Clumit
   Insight의 `trust_registry`가 이 값을 키로 조회하므로 등록된
   값과 일치해야 합니다.
2. **`clumit_insight_bridge_url`** — `/api/analysis/analyze-bridge`로
   최상위 multipart POST를 받을 Clumit Insight 인스턴스의 베이스
   URL. HTTPS 전용.
3. **`clumit_insight_default_model_name`** — `analyze_params_token` JWS의
   `model_name` 클레임에 들어갈 기본 LLM 공급자/모델 패밀리
   식별자. 비어 있지 않은 문자열이면 구조적으로 유효하며,
   허용되는 카탈로그는 Clumit Insight 측 설정에서 관리합니다.
4. **`clumit_insight_default_model`** — `analyze_params_token` JWS의 `model`
   클레임에 들어갈 기본 LLM 모델 식별자. 형식 규칙은
   `clumit_insight_default_model_name`과 같습니다.
...
- **base64url** (43자, 패딩 없음) — 표준. 정확한 비교가 필요할 때
  이 값을 사용하십시오. Clumit Insight의 환경 등록 화면 값과
  복사해 비교하십시오.
...
3. **전환** — 대기 kid를 활성으로 승격하고 기존 kid를 *이전*으로
   강등합니다. **선결 조건**: 새 kid가 Clumit Insight 신뢰
   레지스트리에 미리 등록되어 있어야 합니다. 등록되지 않으면 새
   kid로 서명된 토큰이 거부됩니다. 화면은 버튼 활성화 전에 확인
   체크박스 동의를 요구합니다.
4. **비활성화** — 디스크에서 이전 kid를 제거합니다. 짧은 보존
   기간(기본 5분 — 컨텍스트 토큰 TTL과 시계 스큐 마진의 합) 이후
   자동 활성화됩니다. Clumit Insight 측의 in-flight 검증이
   재전송으로 완료될 시간을 확보하기 위함입니다.
...
이 AICE 인스턴스를 Clumit Insight에 식별시키는
호스트네임(RFC 1123).
...
Clumit Insight 인스턴스의 베이스 URL, HTTPS 전용. 경로는 정규화
형식(끝 슬래시 없음)으로 저장됩니다. 인증 정보, 쿼리 문자열,
프래그먼트는 거부됩니다.

예: `https://insight.example.com`.

### `clumit_insight_default_model_name`

`analyze_params_token` JWS의 `model_name` 클레임으로 전달되는
기본 LLM 공급자/모델 패밀리 식별자입니다. 비어 있지 않은 256자
이하 문자열이면 구조적으로 유효하며, 허용되는 카탈로그는 Clumit
Insight 측 설정에서 관리합니다.
...
구조적으로 유효하며, 허용되는 카탈로그는 Clumit Insight 측
설정에서 관리합니다.
...
`aice_id`, `clumit_insight_bridge_url`, `clumit_insight_default_model_name`,
`clumit_insight_default_model` 중 어느 것이라도 편집하면 닫을 수 없는
모달이 다음을 안내합니다:

> 이 변경 이후 Clumit Insight에 본 환경을 재등록해야 합니다. 기존
> 등록은 무효화되며, 그 사이에 발급된 컨텍스트 토큰은 거부됩니다.
...
Clumit Insight 연동 화면은 다음 이벤트를 기록합니다:

- `aimer_signing_key.generated`, `.rotated`, `.switched`,
  `.deactivated` — 키페어 라이프사이클 이벤트.
- `aimer_integration_setting.changed` — `aice_id`,
  `clumit_insight_bridge_url`, `clumit_insight_default_model_name`,
  `clumit_insight_default_model` 중 어느 것이라도 변경되면
  `{key, old, new}` 삼중값을 기록합니다.

Insight로 분석 흐름은 브라우저가 백그라운드에서 호출하는
...
- `aimer_analyze_envelope.issued` — 성공. 감사 행은 해결된 고객과
  ...
  출처가 baseline 코퍼스인지 Central Manager 대체 경로인지가 함께 기록되어
...
트랙 아래의 **Clumit Insight로 자동 전달 (로그인 중 5분마다)** 토글은
...
렌더링하여 상황을 요약하고 **Settings → Clumit Insight 연동**으로
...
조치 가능한 고객만 경고하기 위함입니다 — **Settings → Clumit
Insight 연동**의 Phase 2 고객 선택기 자체가 활성 고객으로
