...
![탐지 결과 탭 — 와이어프레임 대체 이미지](../assets/detection-tabs-ko.svg)

!!! note "와이어프레임 대체 이미지"

    위 멀티 탭 일러스트는 SVG 와이어프레임입니다. 주변 결과 영역이
    실시간 REview 데이터에 의존하기 때문에, 시드 데이터가 포함된
    스테이징 환경이 준비되는 대로 실제 캡처로 교체합니다. 자세한
    규정은 `docs/AUTHORING.md`의 "인프라 의존 기능에 대한 스크린샷
    예외" 항목을 참고하세요. 스크린샷 교체 작업은 후속 이슈
    [#335](https://github.com/aicers/aice-web-next/issues/335)에서
    추적하며, 스테이징 환경이 준비되면 이 와이어프레임을 실제 PNG로
    교체합니다.

페이지는 **결과 탭** 여러 개로 구성됩니다. 각 탭은 하나의 필터와
해당 캐시된 결과를 함께 담는 일반 검색 탭이며, 필터 드로어와
활성 칩 바는 항상 현재 탭의 필터를 반영합니다.

페이지에 진입하면 기본 필터(**최근 1시간**)가 적용된 탭 하나가
자동으로 생성되어 즉시 실행됩니다. 추가 탭은 **컨텍스트 전환**
용도이며, 현재 필터를 세부 조정할 때는 새 탭을 만들지 말고
활성 탭의 필터 드로어와 Apply를 사용하세요.

새 탭은 다음 방식으로 생성됩니다.

- 탭 바의 `+` 버튼. 새 탭은 기본 필터로 채워지며 자동 실행되지
  **않습니다** — 쿼리를 실행하려면 Apply를 눌러야 합니다.
- 조사(Investigation) 뷰의 피벗 버튼(Phase Detection-12).
- 저장된 필터 / 추천 필터 활성화(Phase Detection-15 / -16).

동시에 열 수 있는 탭은 최대 **8개**입니다. 한도에 도달하면 `+`
버튼이 비활성화되고 툴팁으로 이유를 설명합니다.

각 탭은 다음 요소를 가집니다.

- **탭 이름.** 기본적으로 필터 요약으로 자동 생성됩니다(예: 최근
  1시간 + 위협 레벨 `HIGH` 범위의 탭은 `Last 1h · High`, 필터가
  없는 탭은 `새 탭`). 이름을 두 번 클릭하면 — 또는 키보드 전용
  사용자는 해당 탭에 포커스를 두고 **F2**를 누르면 — 수동으로
  변경할 수 있습니다. 변경한 이름은 필터를 편집해도 유지되므로
  탭을 다듬는 동안 라벨이 덮어씌워지지 않습니다. 이름이 수동
  변경된 탭에는 자동 요약으로 되돌리는 `↺` 버튼이 함께
  표시되며, 키보드로는 F2 → 입력을 비우고 Enter를 누르면 빈
  이름 커밋이 자동 요약으로 되돌리는 동일한 동작을 수행합니다.
- **닫기 버튼.** `×`를 누르거나 탭에 포커스를 둔 상태에서
  **Delete**를 누르면 탭이 제거되고, 이웃한 탭이 활성 상태가
  됩니다. 마지막 탭을 닫으면 기본 탭이 새로 자동
  생성되므로(페이지 초기 진입과 동일하게 `Last 1 hour`가 자동
  실행되며, Apply 대기 상태의 `+` 탭이 아닙니다) 페이지에 탭이
  하나도 없는 상태는 발생하지 않습니다.

탭을 전환해도 네트워크 요청이 발생하지 **않습니다** — 대상 탭의
캐시된 결과가 즉시 표시되고, 필터 드로어와 칩 바가 대상 탭의
필터에 맞춰 재동기화됩니다. 활성 탭의 Apply는 해당 탭의 캐시된
결과만 교체하며, 다른 탭은 영향을 받지 않습니다. 탭을 떠날 때
그 탭의 필터 드로어에 남아 있던 미적용 편집본은 함께 폐기되므로,
다시 돌아와 드로어를 열면 버려진 편집 대신 커밋된 필터가 다시
시드됩니다.

결과 헤더의 **N분 전 업데이트** 라벨은 해당 탭의 마지막 성공한
쿼리를 기준으로 표시되며, 같은 새로고침 버튼이 활성 필터를 다시
실행합니다. 아직 Apply하지 않은 `+` 탭에서는 새로고침이
비활성화되어 있어, 새 탭의 첫 쿼리는 반드시 드로어의 Apply
경로를 거치게 됩니다 — 이슈의 "the user must Apply" 규칙과
일치합니다. 새로고침은 활성 탭의 상대 기간을 실행 시점의
벽시계에 맞춰 다시 계산하므로, 한 시간 전에 Apply한 `최근 1시간`
탭도 지금을 기준으로 한 최근 한 시간을 조회합니다. 새로고침
이후 드로어를 다시 열면 재계산된 창이 그대로 반영됩니다 —
드로어의 start / end 입력은 기간이 앞으로 굴러갈 때 커밋된
필터로부터 다시 시드되므로 칩 바와 드로어가 엇갈리지 않습니다.

기간 칩을 `×`로 제거하면 어느 상태에서든 시간 필터가 완전히
해제됩니다. 아직 Apply하지 않은 `+` 탭에서는 커밋된 기간이나
범위 없이 대기 상태를 유지하고, 이미 실행된 탭에서는 시간
제한 없이 쿼리가 즉시 다시 실행됩니다. 새로고침한 뒤에도 칩이
되돌아오지 않습니다 — 커밋된 탭은 URL에 `?notime=1` 마커를
실어 SSR 로더가 "시간 필터를 의도적으로 비운 상태"임을 인식하고
`최근 1시간`을 다시 살리지 않도록 합니다. 대기 중인 탭은 같은
이유로 `?pending=1`을 실어 나르며, `sessionStorage`가 양쪽을
모두 거울처럼 보존해 떠났던 상태 그대로 돌아옵니다.

#### 새로고침 후 복원

탭 상태는 URL과 `sessionStorage`로 분리해 저장합니다.

- **URL**은 공유 가능한 상태를 담습니다 — 링크 수신자(Slack /
  북마크 / 새로고침)가 재현해야 하는 모든 정보. 활성 탭 인덱스
  (`?tab=N`)와 활성 탭의 **전체 필터** — 기간(또는 명시적
  start/end), 방향, 신뢰도 범위, 범주형 선택(레벨, 국가, 분류,
  종류, 학습 방식), 센서, 엔드포인트 행, 출발지 / 목적지 /
  키워드 / 호스트명 / 사용자 필드 — 가 항상 최상위 파라미터로
  포함됩니다. 작업 탭 전체가 적절한 URL 길이 예산 안에 들어오면
  모든 탭의 필터도 `?tabs=<json>` 파라미터에 함께 실려 공유 링크
  수신자가 활성 탭뿐 아니라 작성자의 탭 전체를 보게 됩니다.
  기간 칩은 로드 시점에 "지금"까지 재계산되므로, `Last 1 hour`
  공유 링크는 수신자 측에서도 "지난 1시간"을 의미합니다. 아직
  쿼리를 실행하지 않은 `+` 탭에는 `?pending=1`이 붙어 새로고침
  후에도 기본 1시간 쿼리가 자동으로 실행되지 않고 쿼리 전 빈
  패널이 그대로 유지됩니다. 향후 쿼리 언어 탭은
  `?mode=query&q=<텍스트>` 형태로 직렬화되며, URL 레이어는
  추상 `Filter` 타입의 두 분기를 모두 왕복하도록 만들어져 있어
  검색 언어 프런트엔드가 연결될 때 지속성 계약을 수정할 필요가
  없습니다.
- **`sessionStorage`**는 공유하지 않는 사설 상태를 담습니다 —
  모든 탭의 필터 스냅샷(수동 이름과 자동 실행 여부 플래그 포함),
  URL 예산을 초과해 싣지 못한 추가 탭, 그리고 탭별 분석 스트립
  펼침 상태가 여기에 저장됩니다. 스크롤 위치, 행 팝오버, 퀵피크
  사이드 패널 등 그 밖의 탭별 UI 상태는 런타임 전용이라 새로
  고침 시 복원되지 않습니다. 저장되는 상태는 공유 링크에 포함되지
  않으므로, 다른 사용자의 패널 배치가 노출되는 일도 없습니다.

페이지 로드 시에는 URL이 우선합니다. `?tabs=<json>`이 전체 탭을
담고 있으면 그 값이 탭 스트립 전체의 정답이며, 세션 스토리지는
분석 스트립 펼침 상태와 URL 측에 이름이 비어 있을 때의 수동
이름을 덧씌우는 용도로만 참조됩니다. URL이 활성 탭만 기술할 때
— 작성자의 작업 탭이 예산을 초과한 공유 링크거나 이벤트 조사
피벗 링크인 경우 — 저장된 탭 세트는 해당 탭을 기준으로 재배치
됩니다. 다른 사용자가 단일 탭 공유 URL을 자신의 브라우저에서
열면 그 한 탭만 보입니다 — 해당 브라우저의 `sessionStorage`는
비어 있기 때문입니다. 작업 중인 탭 세트가 쌓인 뒤 페이지를
새로고침하면 전체 탭과 활성 탭 인덱스가 복원됩니다. 또한 활성
탭이 바뀔 때마다(`+`와 닫기 포함) URL이 즉시 갱신되므로, 새로
고침 시 새 탭이 이전 탭의 필터를 이어받는 일이 없습니다.
...
그때까지는 URL / sessionStorage 디코더가 `mode: "query"` 브랜치를
전방 호환성용으로 계속 라운드트립하지만, 페이지 경계에서 기본
기간의 구조화 탭으로 다운그레이드되므로 칩 바와 드로어가 렌더
불가능한 필터 상태로 빠지지 않습니다.
...
드로어의 모든 필드가 URL에 저장되므로 새로고침이나 공유 링크
모두 활성 탭의 필터를 빠짐없이 복원합니다. 배열(태그 입력,
범주형 선택, 센서)은 쉼표로 구분된 값으로 저장되고
(`?keywords=alpha,beta`), 시간 범위는 기간 칩 선택 시
`?period=1h`로, 명시적 창은 `?start=…&end=…`로 실립니다.
필드를 비우면 해당 파라미터가 URL에서 제거됩니다.
...
Quick peek는 커밋된 쿼리 전환이 **디스패치되는 순간** — 적용,
칩 제거, 또는 새로고침 — 에 즉시 닫힙니다. 교체된 결과 슬라이스가
응답으로 돌아올 때까지 기다리지 않습니다. 인스펙터와 **Open
investigation** 버튼은 새 칩 바 / URL과 동기적으로 사라지므로,
왕복 중에 현재 커밋된 필터가 더 이상 기술하지 않는 행에서 핸드오프가
실행될 여지가 없습니다. REview는 아직 안정적인 이벤트별 식별자를
제공하지 않으므로 인스펙터를 커밋된 쿼리 너머로 유지하면 다른
이벤트를 조용히 가리킬 위험이 있습니다. 모든 커밋 시점에 닫는
방식은 이러한 위험을 막는 방어적 선택이며, 데스크톱 너비에서는
선택된 행이 없어지면 히어로 리스트의 전체 폭도 함께 복구됩니다.
필터가 바뀐 뒤 원하는 행의 Quick peek를 다시 여는 것은 클릭 한
번이면 됩니다.
