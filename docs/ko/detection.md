...
Quick peek는 결과 집합을 교체하는 커밋된 쿼리 전환 — 적용,
칩 제거, 새로고침, 또는 결과 없음 / 오류 상태로의 전환 — 이
일어날 때마다 닫힙니다. REview는 아직 안정적인 이벤트별 식별자를
제공하지 않으므로 인스펙터를 커밋된 쿼리 너머로 유지하면 다른
이벤트를 조용히 가리킬 위험이 있습니다. 모든 커밋 시점에 닫는
방식은 **Open investigation** 핸드오프가 더 이상 유효하지 않은
행에 연결되지 않게 막고, 데스크톱 너비에서는 선택된 행이 없어지면
히어로 리스트의 전체 폭도 함께 복구됩니다. 필터가 바뀐 뒤 원하는
행의 Quick peek를 다시 여는 것은 클릭 한 번이면 됩니다.
