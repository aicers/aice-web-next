...
Quick peek는 현재 페이지 슬라이스 내의 위치가 아니라, 결과
리스트의 키로 사용되는 서버 측 커서로 선택 행을 추적합니다.
필터를 적용하거나, 칩을 제거하거나, 리스트를 새로고침하면
인스펙터는 새 결과 집합과 재조정됩니다. 같은 행이 여전히
존재하면 요약이 그 자리에서 갱신되고, 행이 사라지거나 결과가
없음 / 오류 상태가 되면 인스펙터가 닫혀서 **Open investigation**
핸드오프가 더 이상 유효하지 않은 이벤트에 연결되지 않습니다.
데스크톱 너비에서는 선택된 행이 없어지면 히어로 리스트도 원래
전체 폭으로 복구됩니다.
