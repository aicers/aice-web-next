...
불러오는 중…** 안내가 표시되고, 일시적인 가져오기 실패 시에는
**고객사 목록을 불러오지 못했습니다** 메시지와 함께 인라인
**다시 시도** 버튼이 표시되므로 드로어를 닫고 다시 열지 않아도
복구할 수 있습니다. 북마크된 `?f=` URL, 저장된 필터, 피벗
링크에서 첫 페인트에 등장하는 고객사 칩은 짧은 가져오기 대기
구간 동안에도 원시 ID 대신 고객사 **이름**으로 렌더됩니다 —
페이지가 SSR 단계에서 산출한 고객사 범위 페이로드를 칩 이름
조회의 폴백으로 재사용하기 때문입니다.
...
> 실패가 발생합니다. 백엔드(REview)도 자체 교차 검사를 수행하지만,
> aice-web-next 측은 더 이상 REview를 유일한 시행 지점으로
> 두지 않습니다.
