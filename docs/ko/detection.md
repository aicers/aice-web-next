...
**추천 필터** 섹션에는 폭넓은 시작점을 클릭 한 번으로 적용할 수
있도록 사전 구성된 프리셋이 들어 있습니다. 처음 들어온 사용자가
필터를 처음부터 만들 필요 없이 바로 결과가 채워진 화면에
도달할 수 있게 해 줍니다. v1 초기 목록은 엄브렐러 이슈에서 합의한
넓은 조회 범위 프리셋입니다.
...
행을 활성화하면 프리셋이 **새 탭**에서 열리고(저장된 필터의
기본 활성화 동작과 같습니다), 쿼리가 자동 실행되어 새 탭이
...
레일에는 이름 바꾸기 / 삭제 / 저장 어포던스가 노출되지 않습니다.
테넌트별 추천 목록과 추천 필터 관리 UI는 향후 작업으로
추적합니다.

![추천 필터 레일](../assets/detection-recommended-filters-rail-ko.png)

#### 필터 저장하기

필터 드로어를 열어 보존하고 싶은 필터를 구성한 다음, 드로어
하단의 **이 필터 저장**을 누릅니다. 이름 입력 대화상자가 필터
요약(예: `Last 1h · High`)으로 자동 생성된 이름과 함께 열립니다.
제안된 이름을 그대로 사용하거나 새 이름을 입력한 뒤 **저장**을
누릅니다.

이름은 개인용이며 계정 내에서 고유해야 합니다. 같은 이름을
재사용하면 대화상자에 인라인 오류가 표시됩니다. 저장은 쿼리를
...
**저장된 필터** 레일의 각 행은 상호작용에 따라 작업 공간에
영향을 줍니다.

- **행을 클릭**(또는 행의 `⋮` 메뉴에서 **새 탭에서 불러오기** 선택)
  — 기본 동작 — 은 저장된 필터로 사전 구성된 새 탭을 열고 쿼리를
  자동 실행합니다. 현재 탭은 그대로 유지되며, 이는 탭이
  컨텍스트 전환을 위한 것이라는 탭 바 규약과 일치합니다.
- **현재 탭에서 불러오기**(`⋮` 메뉴)는 활성 탭의 필터를 저장된
  필터로 교체하고 쿼리를 다시 실행합니다. 활성 탭의 페이징은
  첫 페이지로 초기화되며, 그 외 작업 공간 상태(분석 스트립
  확장, Quick peek 선택)는 일반 Apply 규약을 따릅니다.

필터는 시간 범위, 위협 수준, 센서, 출발지 / 목적지, 방향, 신뢰도
범위, 국가 / 카테고리 / 종류 / 학습 방식 선택, 그리고 필터
페이로드에 포함된 네트워크 / IP 엔드포인트 등 모든 커밋된 필드와
함께 적용됩니다.

이미 탭이 8개 열려 있는 상태에서 행을 클릭하거나 **새 탭에서
불러오기**를 선택하면, 레일은 조용히 무시되지 않고 피벗 경로와
동일한 "탭을 닫아야 합니다 — 이미 8개 한도에 도달했습니다"
토스트를 표시합니다. **현재 탭에서 불러오기**는 새 탭을 만들지
않으므로 탭 한도의 영향을 받지 않습니다.

#### 이름 바꾸기와 삭제

각 저장된 필터 행의 `⋮` 메뉴에는 다음 항목도 있습니다.
...
- 탭 바 오른쪽의 `+` 어포던스는 동일한 기본 필터로 새 탭을
  ...
  채워집니다.
- 슬림 좌측 레일에서 **저장된 필터**나 **추천 필터**를
  활성화(기본 클릭 — 위의 "저장된 필터"·"추천 필터" 항목 참고)
  하거나 **피벗 링크**(Phase Detection-12)를 따라가면 현재 탭을
  덮어쓰는 대신 대상 필터로 사전 구성된 새 탭이 만들어집니다.
...
이름이 채워진 입력 대화상자가 열리고, 제출하면 개인 **저장된
필터** 레일에 저장됩니다. 전체 저장 / 불러오기 / 이름
바꾸기 / 삭제 흐름은 위의 "저장된 필터" 항목을 참고하세요.
