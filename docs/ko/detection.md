...
항목을 선택하면 프리셋이 **새 탭**에서 열리고(저장된 필터의
기본 클릭 동작과 같습니다), 쿼리가 자동 실행되어 새 탭이
결과로 채워진 채 열립니다. 현재 탭은 그대로 유지되며 피벗
경로나 저장된 필터 활성화와 동일하게 동작합니다. 탭 8개 한도에
도달하면 작업 공간 다른 곳과 같이 "탭을 닫아야 합니다 — 이미
8개 한도에 도달했습니다" 토스트가 표시되며, 조용히 무시되지
않습니다.
...
- **항목을 클릭** — 기본 동작 — 은 저장된 필터로 사전 구성된
  새 탭을 열고 쿼리를 자동 실행합니다. 현재 탭은 그대로
  유지되며, 이는 탭이 컨텍스트 전환을 위한 것이라는 탭 바
  규약과 일치합니다.
- 행 서브메뉴의 **새 탭에서 불러오기**는 동일한 기본 동작을
