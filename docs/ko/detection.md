...
행 본문을 클릭하면 페이지 오른쪽에 **간단 미리보기**
인스펙터가 열립니다. 간단 미리보기는 이벤트 요약(심각도, 시간,
종류, 신뢰도, 출발지 → 목적지, 센서)과 전체 조사 보기로
이동하는 **조사 열기** 버튼을 제공합니다. 행 끝의 `›`
아이콘은 간단 미리보기를 건너뛰고 바로 **조사 보기**를 엽니다.
조사 보기로의 이동은 로캘을 존중하며, 뒤로 가기가 활성
필터의 칩 상태까지 포함해 원래 Detection 탭으로 돌아가도록
`returnTo` URL 파라미터를 함께 전달합니다. 지원되는
값(IP, 국가, 종류, 분류, 수준, 호스트명, 사용자)의 피벗
링크는 좁혀진 탭을 열거나 포커스합니다 — 이 연결은 피벗
단계에서 제공됩니다.
