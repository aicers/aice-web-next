...
가능한 칩으로도 표시됩니다. 고객사 목록은 탐지 페이지와 함께
배달되므로 드로어를 처음 열어도 별도의 가져오기 대기 없이 바로
채워진 상태로 표시됩니다. 패널 헤더의 작은 **↻** 새로고침
아이콘은 고객사 목록을 명시적으로 다시 가져오며, 다른 브라우저
탭에서 관리자가 계정-고객사 할당을 방금 변경한 경우 유용합니다.
자동 새로고침은 없으며, 캐시된 목록은 현재 페이지 세션의 모든
필터 탭에서 공유되고 모든 드로어 열기에서 재사용됩니다.
...
필터에는 `customers` 값이 전송되지 않습니다. 드로어는 첫 열기
시 고객사 가져오기를 기다리지 않지만(목록은 SSR 페이로드에
포함되어 옴), 수동 **↻** 새로고침을 누르면 명시적인 재가져오기
중에 잠깐 **고객사 목록을 불러오는 중…** 표시가 나타납니다.
일시적인 가져오기 실패 시에는 **고객사 목록을 불러오지
못했습니다** 메시지와 함께 인라인 **다시 시도** 버튼이
