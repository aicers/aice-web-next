...
- **두 번째 줄** — `출발지 → 목적지` 형식의 엔드포인트가
  ...
  사용자 / 호스트 셀은 이벤트 서브타입이 REview 스키마에 따라
...
표시됩니다. 이벤트 서브타입이 REview 스키마에 따라 해당 필드를
제공할 때만 셀이 피벗 가능합니다.
...
추적합니다. REview가 이벤트 페이로드를 확장해 해당 필드를
...
- **페이지당 행**은 `25`, `50`, `100` 중에서 고를 수 있으며
  기본값은 `50`이고, `100`은 REview 0.47.0이 Relay `first` / `last`
...
- **출발지 IP** (`origAddr`)
- **목적지 IP** (`respAddr`)
- **국가** — 출발지·목적지 국가 코드 합산. REview 센티넬 코드
...
띄웁니다. 국가 행은 클릭할 수
없는데, REview의 `eventCountsByCountry`가 출발지·목적지 행을
...
- **빈 결과** — 두 쿼리 모두 0건을 반환했을 때. 결과 목록이 이미
  ...
  대신 필터를 넓히도록 안내합니다.
- **오류** — REview가 한쪽 쿼리를 거부했을 때. `다시 시도` 버튼은
...
> 행동 가능한 안내가 표시됩니다. 백엔드(REview)도 자체 교차
> 검사를 수행하지만, aice-web-next 측은 더 이상 REview를 유일한
> 시행 지점으로 두지 않습니다.
...
- **위협 수준** — `낮음` / `중간` / `높음` (백엔드의 `levels: [LOW,
  MEDIUM, HIGH]`에 매핑). REview의 `ThreatLevel` enum에는 `VERY_LOW`와
...
진행 중인 내보내기를 취소하면 이제 현재 REview 페이지가 끝날
...
진행 중인 REview 왕복 호출에 수십 초가 더 소비될 수 있었습니다.
