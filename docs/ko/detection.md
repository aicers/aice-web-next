...
영역)을 활성화하면 해당 필드에 포커스가 맞춰진 채 필터 드로어가
열려 값을 수정한 뒤 다시 적용할 수 있습니다.
...
행 본문을 클릭하면 **간단 미리보기** 인스펙터가 열립니다(이후
단계에서 추가). 행 끝의 인라인 `›` 아이콘은 전체 **조사 보기**를
엽니다. 지원되는 값(IP, 국가, 종류, 분류, 수준, 호스트명,
사용자)의 피벗 링크는 좁혀진 탭을 열거나 포커스합니다 — 이
연결은 피벗 단계에서 제공됩니다.
