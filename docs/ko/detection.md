...
![피벗 어포던스 — 와이어프레임](../assets/detection-pivot-ko.svg)

!!! note "와이어프레임 대체 이미지"

    위 그림은 실제 캡처가 아닌 SVG 와이어프레임입니다. 피벗
    셀은 REview가 반환하는 실시간 탐지 행 위에 표시되는데,
    문서 작성 워크트리에는 시드 데이터가 있는 스테이징 백엔드가
    없습니다. `docs/AUTHORING.md`의 “인프라 종속 기능에 대한
    스크린샷 예외”에 따라 이 섹션은 다국어 SVG 와이어프레임으로
    먼저 출하되며, 스테이징이 준비되면 실제 캡처로 교체됩니다.
