...
가능한 칩으로도 표시됩니다. 패널 헤더의 작은 **↻** 새로고침
아이콘은 고객사 목록을 다시 가져오며, 다른 브라우저 탭에서
관리자가 계정-고객사 할당을 방금 변경한 경우 유용합니다.
자동 새로고침은 없으며, 캐시된 목록은 현재 페이지 세션의
모든 드로어 열기에서 재사용됩니다.
...
필터에는 `customers` 값이 전송되지 않습니다. 첫 드로어 열기에서
고객사 목록을 가져오는 동안에는 **고객사 목록을 불러오는 중…**
표시가, 일시적인 가져오기 실패 시에는 **고객사 목록을 불러오지
못했습니다** 메시지와 함께 인라인 **다시 시도** 버튼이
나타나므로 드로어를 닫고 다시 열지 않아도 복구할 수 있습니다.
