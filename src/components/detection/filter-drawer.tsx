...
/**
 * Drawer-editable field names a chip can redirect focus to. Text and
 * tag fields resolve to an `<input>` and are focused directly; the
 * remaining keys name a section inside the drawer and are scrolled
 * into view so the chip body always has a meaningful landing zone.
 */
...
export function FilterDrawer({
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      ...
      // Only call focus() on elements that model an interactive
      // control. Section wrappers (fieldsets, divs) don't — calling
      // focus() on them is a no-op at best and shifts focus off the
      // intended target at worst.
      const focusable =
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    // No-time is a valid committed state (operator cleared the Period
    // chip). Reject only asymmetric pairs (one side set, the other
    // blank) and inverted ranges — both sides null is allowed and
    // submits a filter without `start` / `end`.
    const startSet = Boolean(draft.startIso);
    const endSet = Boolean(draft.endIso);
    if (startSet !== endSet) {
    ...
    if (
      startSet &&
      endSet &&
      Date.parse(draft.startIso as string) >= Date.parse(draft.endIso as string)
    ) {
      setValidationError(labels.invalidRange);
      return;
    }
        ...
        >
          ...
          >
            ...
            <fieldset className="flex flex-col gap-3">
              ...
              <div id={`${fieldIdPrefix}-countries`}>
                <FilterMultiSelect
                  id="filter-countries"
              ...
              <div id={`${fieldIdPrefix}-learningMethods`}>
                <FilterMultiSelect
              ...
              <div id={`${fieldIdPrefix}-categories`}>
                <FilterMultiSelect
