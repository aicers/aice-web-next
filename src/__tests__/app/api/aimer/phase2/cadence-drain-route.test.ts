...
function makeReq(body: Record<string, unknown>): NextRequest {
  return new NextRequest("http://localhost/api/aimer/phase2/cadence-drain", {
    method: "POST",
...
describe("POST /api/aimer/phase2/cadence-drain", () => {
  beforeEach(() => {
  ...
  it("returns 403 when not a System Administrator", async () => {
    ...
    const res = await POST(
      makeReq({ customer_id: 42, kind: "story", delivered: 1, no_op: 0 }),
      ctx,
    );
  ...
  it("records cadence_drain with browser-reported counts and returns 204", async () => {
    const { POST } = await import("@/app/api/aimer/phase2/cadence-drain/route");
    const res = await POST(
      makeReq({
        customer_id: 42,
    ...
    expect(mockAuditRecord).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        target: "customer",
        targetId: "42",
        details: { kind: "baseline_event", delivered: 12, noOp: 3 },
      }),
    );
  });
