...
describe("buildAppliedFilter", () => {
  it("throws when the draft's ISO range is asymmetric", () => {
    // A single-sided ISO range is a drawer bug — both-null (no time
    // constraint) and both-set (a concrete range) are the only two
    // valid shapes. The guard keeps a half-cleared pair from silently
  ...
  it("commits a no-time filter when both ISO fields are null", () => {
    // Regression anchor: removing the Period chip leaves the draft in
    // a fully-cleared time state. Apply must accept it and strip any
    // prior `start` / `end` instead of rejecting the operator's edits.
    ...
    const result = buildAppliedFilter(
      committed,
      { ...BASE_DRAFT, startIso: null, endIso: null },
      false,
    );
    if (result.mode !== "structured") throw new Error("unreachable");
    expect(result.input.start).toBeUndefined();
    expect(result.input.end).toBeUndefined();
  });
  ...
  it("preserves a prior `sensors` when the endpoint is not live", () => {
    // Regression (round 13): when the sensor endpoint is loading,
    // unavailable, or failed, the drawer's sensor control is
    // disabled — the operator has no way to interact with it.
    // `buildAppliedFilter` must not silently strip a previously
    // committed `sensors` filter in that case, or any unrelated
    // edit (Level, Source, …) would broaden the query behind the
    // operator's back. Explicit clears flow through the live path
    // below, which is the only way the selection should disappear.
    const committed: Filter = {
      mode: "structured",
    ...
    if (result.mode !== "structured") throw new Error("unreachable");
    expect(result.input.sensors).toEqual(["old-1", "old-2"]);
  });
