 ...
 * `/event` would match a `/events/<token>` detection-event detail page
 * (#678). Matching on path-segment boundaries instead means `/event`
 * only matches `/event` or `/event/...`, never `/events/...`, while
 * `/detection` still matches its nested `/detection/events/<token>`
 * investigation view.
 *
