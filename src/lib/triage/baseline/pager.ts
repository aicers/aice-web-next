 ...
 *
 *   d2. INSERT all surviving (post-exclusion, post-Blocklist*) page
 *       events into `observed_event_meta` with
 ...
 *   e4. INSERT all page rows into `baseline_triaged_event` with
...
export function createCadencePager(
  ...
  return {
    async ingestPage(
      ...
      // Page rows must be in the denominator before Phase 2 reads it
      // — see the RFC's "S3 / S4 already account for self via `- 1`"
      // contract. ON CONFLICT keeps the path idempotent against a
      // re-run of the same page after a Phase 2 failure.
      let observedInserted = 0;
      for (const { eventKey, event, cols } of survivors) {
      ...
      // cutoffs, not on a stored INSERT gate).
