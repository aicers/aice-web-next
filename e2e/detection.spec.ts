...
test("drawer renders Customer placeholder and Sensor fallback while REview endpoint is absent", async ({
  ...
  // While REview can't provide a sensor list, the control's primary
  // trigger (the first button in the fieldset) is disabled across
  // every non-ready state — `unavailable` (endpoint absent),
  // `loading`, and `error`. The `error` state additionally renders a
  // Retry button next to the trigger, so we target `.first()` to
  // keep this assertion stable regardless of which non-ready state
  // the worker account happens to land in (workers without a
  // populated customer scope currently hit `error` because
  // `listSensors` rejects empty-scope callers before the
  // endpoint-absent fallback runs).
